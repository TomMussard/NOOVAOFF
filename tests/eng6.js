...
(async()=>{
  ...
  await T('config admin',async()=>{
    ...
    check('Réglages : l\'admin lit valeurs, défauts et bornes (reveal : pas de seuil par défaut, 1)',g.values['REVEAL.MIN_ANSWERS']===1&&g.defaults['REVEAL.MIN_ANSWERS']===1&&g.bounds['REVEAL.MIN_ANSWERS'].min===1&&Object.keys(g.bounds).length===7&&!Object.keys(g.bounds).some(k=>/^PREDICTION|^COMMUNITY|^QUOTA/.test(k)),g);
    ...
    check('Par défaut : pas de seuil, pourcentages visibles dès 3 réponses',!r.belowThreshold&&r.options.map(o=>o.pct).join()==='67,33,0',r);
    check('Valeur hors bornes refusée (0 < 1)',(await callFn('setEngagementConfig',{values:{'REVEAL.MIN_ANSWERS':0}},tokAdmin)).error==='INVALID_ARGUMENT');
    ...
    const s1=await callFn('setEngagementConfig',{values:{'REVEAL.MIN_ANSWERS':5,'COMPAT.MIN_COMMON':5}},tokAdmin);
    check('Enregistrement : l\'admin peut quand même relever le seuil s\'il le souhaite',s1.values['REVEAL.MIN_ANSWERS']===5&&s1.values['COMPAT.MIN_COMMON']===5,s1);
    ...
    check('Après relèvement à 5 : 3 réponses < 5 → pas de pourcentage',r.belowThreshold===true&&r.needed===2,r);
    const s2=await callFn('setEngagementConfig',{values:{'REVEAL.MIN_ANSWERS':null,'COMPAT.MIN_COMMON':null}},tokAdmin);
    await sleep(800);
    r=await E.revealCore('me',{campaignId:'c1',questionIdx:0});
    check('Retour aux défauts (null) : pas de seuil rétabli (1)',s2.values['REVEAL.MIN_ANSWERS']===1&&!r.belowThreshold);
    // interface admin
