...
(async()=>{
  ...
  await T('config admin',async()=>{
    ...
    check('Réglages : l\'admin lit valeurs, défauts et bornes (reveal 5 par défaut)',g.values['REVEAL.MIN_ANSWERS']===5&&g.defaults['REVEAL.MIN_ANSWERS']===5&&g.bounds['REVEAL.MIN_ANSWERS'].min===2&&Object.keys(g.bounds).length===9,g);
    ...
    await wf(p,()=>document.querySelectorAll('#eng-cfg-rows input').length===9,null,15000);
