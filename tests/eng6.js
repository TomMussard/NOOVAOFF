...
(async()=>{
  ...
  await T('config admin',async()=>{
    ...
    check('Réglages : l\'admin lit valeurs, défauts et bornes (reveal 5 par défaut)',g.values['REVEAL.MIN_ANSWERS']===5&&g.defaults['REVEAL.MIN_ANSWERS']===5&&g.bounds['REVEAL.MIN_ANSWERS'].min===2&&Object.keys(g.bounds).length===8,g);
    ...
    await wf(p,()=>document.querySelectorAll('#eng-cfg-rows input').length===8,null,15000);
    ...
    check('Admin (interface) : la modification est enregistrée avec l\'auteur',(await db.doc('appConfig/engagement').get()).data().PREDICTION.MIN_ANSWERS===12&&(await db.doc('appConfig/engagement').get()).data().updatedBy==='tomussproduction@gmail.com');
    await p.evaluate(()=>{window.confirm=()=>true;resetEngagementConfig();});
    await until(async()=>!((await db.doc('appConfig/engagement').get()).data()||{}).PREDICTION);
    check('Admin (interface) : « valeurs par défaut » efface les réglages',!((await db.doc('appConfig/engagement').get()).data()||{}).PREDICTION);
    await b.close();
