...
(async () => {
  ...
  await T('cap', async () => {
    ...
    check('Plafond : jamais plus de 2 par jour, même exceptionnel (reportée au lendemain)', r2.status === 'queued', r2);
    check('Plafond : toujours 2 envoyées', (await sink('u')).length === 2);
    await N.drainQueue(at(addDays(D1, 1), '09:00'));
    const s2 = await sink('u');
    check('Reportée : envoyée le lendemain à 9h', s2.length === 3 && s2[2].ntype === 'recompense_debloquee', s2.map(x => x.ntype));
  });
  await T('quiet', async () => {
    await wipe();
    await mkUser('u');
