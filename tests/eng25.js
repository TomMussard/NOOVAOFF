...
const ansX=(u,cid,opt)=>db.doc(`answers/${u}_${cid}_q0`).set({userId:u,campaignId:cid,merchantId:'m1',questionIdx:0,answer:['Café','Thé','Chocolat'][opt],optionIdx:opt,respondentCity:'le-mans',category:'boulangerie',flagged:false,pointsAwarded:0,createdAt:Timestamp.now()});
...
(async()=>{
  ...
  await T('fil',async()=>{
    ...
    const order=f.cards.map(c=>/cm-compat/.test(c.cls)?'C':/cm-k-top/.test(c.cls)?'T':/cm-k-new/.test(c.cls)?'N':/cm-level/.test(c.cls)?'L':/cm-news/.test(c.cls)?'A':'?').join('');
    check('Le fil est varié : plusieurs natures de cartes, la compatibilité en 2e position',new Set(order.split('')).size>=4&&order[1]==='C',order);
