...
(async () => {
  ...
  await step('merchant register 2 steps', async () => {
    ...
    await setVal(mp, '#aw-city', 'Le Mans'); await setVal(mp, '#aw-address', '12 rue Gambetta'); await setVal(mp, '#aw-siret', '12345678901234');
    await mp.evaluate(() => awSubmit());
  ...
  await step('merchant reach + launch + cashier', async () => {
    ...
    check('C6 durée de campagne : endsAt posé à 30 jours', camps.docs[0].data().endsAt && Math.abs(camps.docs[0].data().endsAt.toMillis() - Date.now() - 30*86400000) < 3600000);
    await sleep(1500);
